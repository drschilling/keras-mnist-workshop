...
plt.style.use(["ggplot", "kerasviz.mplstyle"])
#import matplotlib
...
def kr_format_mnist_images(X, Y, Y_, n=100, lines=10):
    
    correct_prediction = kr.equal(kr.argmax(Y,1), kr.argmax(Y_,1))
    correctly_recognised_indices = kr.squeeze(kr.where(correct_prediction), [1])  # indices of correctly recognised images
    incorrectly_recognised_indices = kr.squeeze(kr.where(kr.logical_not(correct_prediction)), [1]) # indices of incorrectly recognised images
    everything_incorrect_first = kr.concat([incorrectly_recognised_indices, correctly_recognised_indices], 0) # images reordered with indeces of unrecognised images first
    everything_incorrect_first = kr.slice(everything_incorrect_first, [0], [n]) # compute first 100 only - no space to display more anyway
    # compute n=100 digits to display only
    Xs = kr.gather(X, everything_incorrect_first)
    Ys = kr.gather(Y, everything_incorrect_first)
    Ys_ = kr.gather(Y_, everything_incorrect_first)
    correct_prediction_s = kr.gather(correct_prediction, everything_incorrect_first)

    digits_left = kr.image.grayscale_to_rgb(kerasviz_digits.digits_left())
    correct_tags = kr.gather(digits_left, kr.argmax(Ys_, 1)) # correct digits to be printed on the images
    digits_right = kr.image.grayscale_to_rgb(kerasviz_digits.digits_right())
    computed_tags = kr.gather(digits_right, kr.argmax(Ys, 1)) # computed digits to be printed on the images
    #superimposed_digits = correct_tags+computed_tags
    superimposed_digits = kr.where(correct_prediction_s, kr.zeros_like(correct_tags),correct_tags+computed_tags) # only pring the correct and computed digits on unrecognised images
    correct_bkg   = kr.reshape(kr.tile([1.3,1.3,1.3], [28*28]), [1, 28,28,3]) # white background
    incorrect_bkg = kr.reshape(kr.tile([1.3,1.0,1.0], [28*28]), [1, 28,28,3]) # red background
    recognised_bkg = kr.gather(kr.concat([incorrect_bkg, correct_bkg], 0), kr.cast(correct_prediction_s, kr.int32)) # pick either the red or the white background depending on recognised status

    I = kr.image.grayscale_to_rgb(Xs)
    I = ((1-(I+superimposed_digits))*recognised_bkg)/1.3 # stencil extra data on top of images and reorder them unrecognised first
    I = kr.image.convert_image_dtype(I, kr.uint8, saturate=True)
    Islices = [] # 100 images => 10x10 image block
    for imslice in range(lines):
        Islices.append(kr.concat(kr.unstack(kr.slice(I, [imslice*n//lines,0,0,0], [n//lines,28,28,3])), 1))
    I = kr.concat(Islices, 0)
    return I
...
class MnistDataVis:
    ...
    def animate(self, compute_step, iterations, train_data_update_freq=20, test_data_update_freq=100, one_test_at_start=True, more_tests_at_start=False, save_movie=False):
        ...
        if save_movie:
            ...
            self._animation.save("./kerasviz_video.mp4", writer=mywriter)
        else:
            plt.show(block=True)